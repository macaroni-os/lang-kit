...
src_unpack() {
	...
	mv "${WORKDIR}/${MY_P}-$(rust_abi)" "${S}" || die
		if use rls; then
				mv "${WORKDIR}/rust-src-1.62.1/rust-src" "${S}"/rust-src
		fi
		if use wasm; then
				mv "${WORKDIR}/rust-std-1.62.1-wasm32-unknown-unknown/rust-std-wasm32-unknown-unknown" "${S}"/rust-std-wasm32-unknown-unknown
		fi
}
